...
export async function POST(req: Request) {
  try {
      ...
      return NextResponse.json(
        {
          success: false,
          error:
            "Accessory title is required and must be at least 3 characters.",
        },
        { status: 400 }
      );
    }
      ...
      return NextResponse.json(
        {
          success: false,
          error:
            "Status is required and must be one of: active, inactive, discontinued.",
        },
        { status: 400 }
      );
    }
    ...
    const existing = await db
      .select()
      .from(accessoryTable)
      .where(eq(accessoryTable.slug, slug));
...
// ===================== DELETE: Delete accessory by id =====================
export async function DELETE(req: Request) {
  const { searchParams } = new URL(req.url);
  const idParam = searchParams.get("id");
  if (!idParam) {
    return NextResponse.json(
      { success: false, error: "Missing accessory id" },
      { status: 400 }
    );
  }
  const id = Number(idParam);
  ...
  try {
    ...
    const message =
      error instanceof Error ? error.message : "Failed to delete accessory";
    return NextResponse.json(
      { success: false, error: message },
      { status: 500 }
    );
