export const pumpsSeedData = [
   {
      title: '0.5 HP Espa Prisma 15 2M Pressure Pump',
      slug: '0-5hp-espa-prisma-15-2m-pressure-pump',
      categoryName: 'Pressure Pumps',
      description: `Prisma 15 series\nHorizontal multistage centrifugal pump.\nSelf-priming up to 2 m.\nDischarge and suction connection: 1".\nMaterials: Pump body & impellers in AISI 304, shaft in AISI 431, diffusers technopolymer, mechanical seal aluminium-graphite.\nProtection: IPX5. Max water temperature: 40Â°C.`,
      imageUrl: '/centrifugal/espa_0.8hp.webp',
      price: 56000,
      discountPrice: null,
      stock: 10,
      brand: 'Espa',
      isFeatured: false,
      status: 'active',
      specs: [
         { id: 0, field: 'Horsepower', value: '0.5HP' },

         { id: 1, field: 'Motor Power (P2)', value: '0.24 kW' },

         { id: 2, field: 'Voltage', value: '230 V' },

         { id: 3, field: 'Protection', value: 'IPX5' },

         { id: 4, field: 'Max Suction', value: '2 m' },

         { id: 5, field: 'Max Working Pressure', value: '6 bar' },

         { id: 6, field: 'Discharge Size', value: '1â€³' },

         { id: 7, field: 'Suction Size', value: '1â€³' },
      ],
   },
   {
      title: '0.5 HP Espa Prisma 15 2M Pressure Pump (Made in China)',
      slug: '0-5hp-espa-prisma-15-2m-pressure-pump-made-in-china',
      categoryName: 'Pressure Pumps',
      description: `Prisma 15 2M (Made in China)\nSame specifications as original model but sourced from China.`,
      imageUrl: '/centrifugal/espa_0.8hp.webp',
      price: 40000,
      discountPrice: null,
      stock: 10,
      brand: 'Espa',
      isFeatured: false,
      status: 'active',
      specs: [
         { id: 0, field: 'Horsepower', value: '0.5HP' },

         { id: 1, field: 'Motor Power (P2)', value: '0.24 kW' },

         { id: 2, field: 'Voltage', value: '230 V' },

         { id: 3, field: 'Protection', value: 'IPX5' },

         { id: 4, field: 'Max Suction', value: '2 m' },

         { id: 5, field: 'Max Working Pressure', value: '6 bar' },

         { id: 6, field: 'Discharge Size', value: '1â€³' },

         { id: 7, field: 'Suction Size', value: '1â€³' },

         { id: 8, field: 'Origin', value: 'Made in China' },
      ],
   },
   {
      title: '0.8 HP Espa Prisma 15 3M Pressure Pump',
      slug: '0-8hp-espa-prisma-15-3m-pressure-pump',
      categoryName: 'Pressure Pumps',
      description: `Prisma 15 series\n3M version: Higher head (30-5 bar range) with same materials and design features.\nSelf-priming to 2 m, IPX5, 1" suction/discharge.\nMotor power P2: 0.37 kW.`,
      imageUrl: '/centrifugal/espa_0.8hp.webp',
      price: 56000,
      discountPrice: null,
      stock: 10,
      brand: 'Espa',
      isFeatured: false,
      status: 'active',
      specs: [
         { id: 0, field: 'Horsepower', value: '0.8HP' },

         { id: 1, field: 'Motor Power (P2)', value: '0.37 kW' },

         { id: 2, field: 'Voltage', value: '230 V' },

         { id: 3, field: 'Protection', value: 'IPX5' },

         { id: 4, field: 'Max Suction', value: '2 m' },

         { id: 5, field: 'Max Working Pressure', value: '6 bar' },

         { id: 6, field: 'Discharge Size', value: '1â€³' },

         { id: 7, field: 'Suction Size', value: '1â€³' },
      ],
   },
   {
      title: '0.8 HP Espa Prisma 15 3M Pressure Pump (Made in China)',
      slug: '0-8hp-espa-prisma-15-3m-pressure-pump-made-in-china',
      categoryName: 'Pressure Pumps',
      description: `Prisma 15 3M (Made in China)\nSame hydraulic & electrical specs as original 3M model.`,
      imageUrl: '/centrifugal/espa_0.8hp.webp',
      price: 40000,
      discountPrice: null,
      stock: 10,
      brand: 'Espa',
      isFeatured: false,
      status: 'active',
      specs: [
         { id: 0, field: 'Horsepower', value: '0.8HP' },

         { id: 1, field: 'Motor Power (P2)', value: '0.37 kW' },

         { id: 2, field: 'Voltage', value: '230 V' },

         { id: 3, field: 'Protection', value: 'IPX5' },

         { id: 4, field: 'Max Suction', value: '2 m' },

         { id: 5, field: 'Max Working Pressure', value: '6 bar' },

         { id: 6, field: 'Discharge Size', value: '1â€³' },

         { id: 7, field: 'Suction Size', value: '1â€³' },

         { id: 8, field: 'Origin', value: 'Made in China' },
      ],
   },
   {
      title: '1.0 HP Espa Prisma 15 4M Pressure Pump',
      slug: '1-0hp-espa-prisma-15-4m-pressure-pump',
      categoryName: 'Pressure Pumps',
      description: `Prisma 15 series\n4M version: Pressure range 40-5 bar. Materials same as other Prisma models.\nSelf-priming to 2 m, IPX5 protection, 1" connections.\nMotor power P2: 0.55 kW.`,
      imageUrl: '/centrifugal/espa_0.8hp.webp',
      price: 56000,
      discountPrice: null,
      stock: 10,
      brand: 'Espa',
      isFeatured: false,
      status: 'active',
      specs: [
         { id: 0, field: 'Horsepower', value: '1.0HP' },

         { id: 1, field: 'Motor Power (P2)', value: '0.55 kW' },

         { id: 2, field: 'Voltage', value: '230 V' },

         { id: 3, field: 'Protection', value: 'IPX5' },

         { id: 4, field: 'Max Suction', value: '2 m' },

         { id: 5, field: 'Max Working Pressure', value: '6 bar' },

         { id: 6, field: 'Discharge Size', value: '1â€³' },

         { id: 7, field: 'Suction Size', value: '1â€³' },
      ],
   },
   {
      title: '1.0 HP Espa Prisma 15 4M Pressure Pump (Made in China)',
      slug: '1-0hp-espa-prisma-15-4m-pressure-pump-made-in-china',
      categoryName: 'Pressure Pumps',
      description: `Prisma 15 4M (Made in China)\nSame performance and build specs as original 4M model.`,
      imageUrl: '/centrifugal/espa_0.8hp.webp',
      price: 40000,
      discountPrice: null,
      stock: 10,
      brand: 'Espa',
      isFeatured: false,
      status: 'active',
      specs: [
         { id: 0, field: 'Horsepower', value: '1.0HP' },

         { id: 1, field: 'Motor Power (P2)', value: '0.55 kW' },

         { id: 2, field: 'Voltage', value: '230 V' },

         { id: 3, field: 'Protection', value: 'IPX5' },

         { id: 4, field: 'Max Suction', value: '2 m' },

         { id: 5, field: 'Max Working Pressure', value: '6 bar' },

         { id: 6, field: 'Discharge Size', value: '1â€³' },

         { id: 7, field: 'Suction Size', value: '1â€³' },

         { id: 8, field: 'Origin', value: 'Made in China' },
         {
            title: 'Taifu TCP130 Centrifugal Pump 0.5HP \u2013 100% Copper Winding',
            slug: 'taifu-tcp130-centrifugal-pump-0.5hp-100percent-copper-winding',
            categoryName: 'Centrifugal Pumps',
            description:
               '0.5HP Centrifugal Pump\r\nDurable and efficient for household and agricultural applications.\r\n100% copper winding ensures reliability and long life.',
            imageUrl: '/centrifugal/Taifu TCP130.webp',
            price: 17000,
            discountPrice: null,
            stock: 10,
            brand: 'Taifu',
            status: 'active',
            isFeatured: false,
            specs: [
               { id: 0, field: 'Horsepower', value: '0.5HP' },
               { id: 1, field: 'Winding', value: '100% Copper' },
            ],
         },
         {
            title: 'Taifu TCP158 Centrifugal Pump 1.0HP \u2013 100% Copper Winding',
            slug: 'taifu-tcp158-centrifugal-pump-1.0hp-100percent-copper-winding',
            categoryName: 'Centrifugal Pumps',
            description:
               '1.0HP Centrifugal Pump\r\nRobust construction suitable for continuous operation.\r\n100% copper winding for enhanced motor life.',
            imageUrl: '/centrifugal/Taifu Tcp 158.webp',
            price: 22999,
            discountPrice: null,
            stock: 10,
            brand: 'Taifu',
            status: 'active',
            isFeatured: false,
            specs: [
               { id: 0, field: 'Horsepower', value: '1.0HP' },
               { id: 1, field: 'Winding', value: '100% Copper' },
            ],
         },
         {
            title: 'Taifu QB70 Peripheral Pump 0.75HP \u2013 100% Copper Winding',
            slug: 'taifu-qb70-peripheral-pump-0-75hp',
            categoryName: 'Pressure Pumps',
            description:
               'Peripheral Pump 0.75HP\nReliable for domestic use.\n100% copper winding for longer motor life.\nCompact, durable, and efficient design.',
            imageUrl: '/centrifugal/Taifu QB70.webp',
            price: 16999,
            discountPrice: null,
            stock: 10,
            brand: 'Taifu',
            status: 'active',
            isFeatured: false,
            specs: [
               { id: 0, field: 'Horsepower', value: '0.75HP' },
               { id: 1, field: 'Winding', value: '100% Copper' },
            ],
         },
         {
            title: 'Taifu TCP200 Centrifugal Pump 2.0HP \u2013 100% Copper Winding',
            slug: 'taifu-tcp200-centrifugal-pump-2.0hp-100percent-copper-winding',
            categoryName: 'Centrifugal Pumps',
            description:
               '2.0HP Centrifugal Pump\r\nDurable design for continuous duty.\r\n100% copper winding ensures long life and efficiency.\r\nIdeal for water supply, irrigation, and pressure boosting.',
            imageUrl: '/centrifugal/taifu_tcp200.webp',
            price: 32999,
            discountPrice: null,
            stock: 10,
            brand: 'Taifu',
            status: 'active',
            isFeatured: false,
            specs: [
               { id: 0, field: 'Horsepower', value: '2.0HP' },
               { id: 1, field: 'Winding', value: '100% Copper' },
            ],
         },
         {
            title: 'Taifu QB60 Peripheral Pump 0.5HP \u2013 100% Copper Winding',
            slug: 'taifu-qb60-peripheral-pump-0.5hp-100percent-copper-winding',
            categoryName: 'Pressure Pumps',
            description:
               'Peripheral Pump 0.5HP\r\nReliable domestic pump for small-scale water transfer.\r\n100% copper winding for durability and efficiency.',
            imageUrl: '/centrifugal/taifu QB60.jpg',
            price: 11499,
            discountPrice: null,
            stock: 10,
            brand: 'Taifu',
            status: 'active',
            isFeatured: false,
            specs: [
               { id: 0, field: 'Horsepower', value: '0.5HP' },
               { id: 1, field: 'Winding', value: '100% Copper' },
            ],
         },
         {
            title: 'Taifu QB80 Peripheral Pump 1.0HP \u2013 100% Copper Winding',
            slug: 'taifu-qb80-peripheral-pump-1.0hp-100percent-copper-winding',
            categoryName: 'Pressure Pumps',
            description:
               'Peripheral Pump 1.0HP\r\nConnection 1"\u00d71"  Max Suction: 8M  45 L/min\r\n1HP QB80 \u2013 Peripheral Pump with 100% copper winding.\r\nReliable for domestic water transfer with good suction and flow rates.',
            imageUrl: '/centrifugal/Taifu QB80.webp',
            price: 18499,
            discountPrice: null,
            stock: 10,
            brand: 'Taifu',
            status: 'active',
            isFeatured: false,
            specs: [
               { id: 0, field: 'Horsepower', value: '1.0HP' },
               { id: 1, field: 'Connection', value: '1"\u00d71"' },
               { id: 2, field: 'Max Suction', value: '8M' },
               { id: 3, field: 'Flow Rate', value: '45 L/min' },
               { id: 4, field: 'Winding', value: '100% Copper' },
            ],
         },
         {
            title: 'Taifu QB90 Peripheral Pump 1.2HP \u2013 100% Copper Winding',
            slug: 'taifu-qb90-peripheral-pump-1.2hp-100percent-copper-winding',
            categoryName: 'Pressure Pumps',
            description:
               '1.2HP Peripheral Pump\r\nConnection 1"x 1"  Max Suction: 8M  Flow Rate: 42 L/M  Max Head: 75M\r\nProduced by advanced high-speed punching line.\r\nAdopting DE carbon mechanical seal, prolong service life 170%.\r\nClass-F copper wire, motor heat resistance up to 155\u00b0C.\r\nAnti-rust, wear-resistant and high precision.\r\n100% rotor fault detection test.\r\nWelded stainless steel rotor shaft.\r\nThickened and reinforced, stable and durable.\r\nCooling sink area increased by 20%.\r\nThickness of motor body and foot increased by 20%.\r\nAdopting Japanese-imported casting line.\r\nHigh strength and corrosion resistant.\r\n100% electrophoresis treatment for anti-rust.\r\nAnti-strike and anti-aging.\r\nReinforcing nylon material.\r\nAnti-drop test 0.8 meters withstand 500g.\r\nBrass.',
            imageUrl: '/centrifugal/Taifu QB90.webp',
            price: 21999,
            discountPrice: null,
            stock: 10,
            brand: 'Taifu',
            status: 'active',
            isFeatured: false,
            specs: [
               { id: 0, field: 'Horsepower', value: '1.2HP' },
               { id: 1, field: 'Connection', value: '1"x 1"' },
               { id: 2, field: 'Max Suction', value: '8M' },
               { id: 3, field: 'Flow Rate', value: '42 L/M' },
               { id: 4, field: 'Max Head', value: '75M' },
               { id: 5, field: 'Winding', value: '100% Copper' },
            ],
         },
         {
            title: 'Pioneer HKS60 Self Priming Pump',
            slug: 'pioneer-hks60-self-priming-pump',
            categoryName: 'Self-Priming Pumps',
            description:
               '0.5HP\r\nApplication: Suitable for use with clean water that does not contain abrasive particles and liquids that are not chemically aggressive towards the materials.\r\nCompact, reliable, and easy to use for domestic applications such as water distribution, garden irrigation, and drawing water from tanks even when air or water may be present.\r\nComes complete with a flap-check valve.\r\nOperating conditions: liquid temperature up to 60\u2103, ambient temperature up to 40\u2103, total suction lift up to 9m.',
            imageUrl: '/centrifugal/pioneer_hks60.webp',
            price: 13500,
            discountPrice: null,
            stock: 10,
            brand: 'Pioneer',
            status: 'active',
            isFeatured: false,
            specs: [
               { id: 0, field: 'Horsepower', value: '0.5HP' },
               { id: 1, field: 'Total Head', value: '30 meter' },
               { id: 2, field: 'Total Suction Lift', value: '9m' },
            ],
         },
         {
            title: 'Pioneer HKS70 Self Priming Pump',
            slug: 'pioneer-hks70-self-priming-pump',
            categoryName: 'Self-Priming Pumps',
            description:
               '0.7HP\r\nApplication: Suitable for use with clean water that does not contain abrasive particles and liquids that are not chemically aggressive towards the materials.\r\nCompact, reliable, and easy to use in domestic applications such as water distribution, garden irrigation, and drawing water from tanks even when air may be present.\r\nComes complete with a flap-check valve.\r\nOperating conditions:\r\nLiquid temperature up to 60\u2103\r\nAmbient temperature up to 40\u2103\r\nTotal suction lift up to 9m.',
            imageUrl: '/centrifugal/Pioneer HKS70.webp',
            price: 18500,
            discountPrice: null,
            stock: 10,
            brand: 'Pioneer',
            status: 'active',
            isFeatured: false,
            specs: [
               { id: 0, field: 'Horsepower', value: '0.7HP' },
               { id: 1, field: 'Total Suction Lift', value: '9m' },
            ],
         },
         {
            title: 'Pedrollo PKm 60 (Made in Italy)',
            slug: 'pedrollo-pkm-60-made-in-italy',
            categoryName: 'Centrifugal Pumps',
            description:
               'Peripheral impeller pump\nDesigned to transfer clean water free from abrasive particles and liquids that will not damage the pump\u2019s components.\nReliable, easy to operate, and cost-effective.\nIdeal for domestic applications, specifically for gardens and orchards, in combination with small autoclave tanks.',
            imageUrl: '/centrifugal/Pedrollo PKm 60.webp',
            price: 28500,
            discountPrice: null,
            stock: 10,
            brand: 'Pedrollo',
            status: 'active',
            isFeatured: false,
            specs: [
               { id: 0, field: 'Flow Rate', value: 'up to 40 l/min (2.4 m\u00b3/h)' },
               { id: 1, field: 'Maximum Head', value: 'up to 40m' },
               { id: 2, field: 'Manometric Suction Head', value: 'up to 8m' },
               { id: 3, field: 'Liquid Temperature', value: '-10\u00b0C to +60\u00b0C' },
               { id: 4, field: 'Ambient Temperature', value: 'up to +50\u00b0C' },
               { id: 5, field: 'Maximum Working Pressure', value: '6 bar' },
               { id: 6, field: 'Motor', value: '2 poles' },
               { id: 7, field: 'Ingress Protection Rating', value: 'IP X4' },
               { id: 8, field: 'Pump Housing', value: 'Cast iron' },
               { id: 9, field: 'Impeller', value: 'Brass' },
               { id: 10, field: 'Shaft', value: 'Stainless steel AISI 431' },
               { id: 11, field: 'Motor Bracket', value: 'Aluminium with brass cover' },
            ],
         },
         {
            title: '1.0 HP Espa Prisma 15 4M Pressure Pump',
            slug: '1-0hp-espa-prisma-15-4m-pressure-pump',
            categoryName: 'Pressure Pumps',
            description:
               'Prisma 15 series\n4M version: Pressure range 40-5 bar. Materials same as other Prisma models.\nSelf-priming to 2 m, IPX5 protection, 1" connections.\nMotor power P2: 0.55 kW.',
            imageUrl: '/centrifugal/espa_0.8hp.png',
            price: 56000,
            discountPrice: null,
            stock: 10,
            brand: 'Espa',
            status: 'active',
            isFeatured: false,
            specs: [
               { id: 0, field: 'Horsepower', value: '1.0HP' },
               { id: 1, field: 'Motor Power (P2)', value: '0.55 kW' },
               { id: 2, field: 'Voltage', value: '230 V' },
               { id: 3, field: 'Protection', value: 'IPX5' },
               { id: 4, field: 'Max Suction', value: '2 m' },
               { id: 5, field: 'Max Working Pressure', value: '6 bar' },
               { id: 6, field: 'Discharge Size', value: '1\u2033' },
               { id: 7, field: 'Suction Size', value: '1\u2033' },
            ],
         },
         {
            title: 'Pedrollo CP158 ST6 Centrifugal Electric Pump (Stainless Steel, 3-Phase)',
            slug: 'pedrollo-cp158-st6-centrifugal-electric-pump',
            categoryName: 'Centrifugal Pumps',
            description:
               'Stainless steel centrifugal pump, 3-phase CP-ST6 series\nSuitable for clean water and non-aggressive liquids.\nAll wet parts in stainless steel AISI 316L for high corrosion resistance.\nUse: domestic, agricultural, industrial water transfer.\nManometric suction lift up to 7 m; liquid temperature -10 \u00b0C to +90 \u00b0C; ambient max +40 \u00b0C.\nProtection: IP X4. Continuous duty (S1).',
            imageUrl: '/centrifugal/Pedrolla 158.webp',
            price: 19000,
            discountPrice: null,
            stock: 10,
            brand: 'Pedrollo',
            status: 'active',
            isFeatured: false,
            specs: [
               { id: 0, field: 'Phase', value: '3-phase' },
               { id: 1, field: 'Horsepower', value: '1 HP' },
               { id: 2, field: 'FlowRate', value: '10 \u00f7 120 l/min' },
               { id: 3, field: 'HeadRange', value: '35.5 \u00f7 19 m' },
               { id: 4, field: 'Max Working Pressure', value: '6 bar' },
               { id: 5, field: 'Manometric Suction', value: '7 m' },
               { id: 6, field: 'Liquid Temp Range', value: '-10 \u00b0C to +90 \u00b0C' },
               { id: 7, field: 'Ambient Temp Max', value: '+40 \u00b0C' },
               { id: 8, field: 'Pump Housing Material', value: 'Stainless Steel AISI 316L' },
               { id: 9, field: 'Impeller Material', value: 'Stainless Steel AISI 316L' },
               { id: 10, field: 'Shaft Material', value: 'Stainless Steel 316L' },
            ],
         },
         {
            title: '0.5 HP Espa Prisma 15 2M Pressure Pump',
            slug: '0-5hp-espa-prisma-15-2m-pressure-pump',
            categoryName: 'Pressure Pumps',
            description:
               'Prisma 15 series\nHorizontal multistage centrifugal pump.\nSelf-priming up to 2 m.\nDischarge and suction connection: 1".\nMaterials: Pump body & impellers in AISI 304, shaft in AISI 431, diffusers technopolymer, mechanical seal aluminium-graphite.\nProtection: IPX5. Max water temperature: 40\u00b0C.',
            imageUrl: '/centrifugal/espa_0.8hp.png',
            price: 56000,
            discountPrice: null,
            stock: 10,
            brand: 'Espa',
            status: 'active',
            isFeatured: false,
            specs: [
               { id: 0, field: 'Horsepower', value: '0.5HP' },
               { id: 1, field: 'Motor Power (P2)', value: '0.24 kW' },
               { id: 2, field: 'Voltage', value: '230 V' },
               { id: 3, field: 'Protection', value: 'IPX5' },
               { id: 4, field: 'Max Suction', value: '2 m' },
               { id: 5, field: 'Max Working Pressure', value: '6 bar' },
               { id: 6, field: 'Discharge Size', value: '1\u2033' },
               { id: 7, field: 'Suction Size', value: '1\u2033' },
            ],
         },
         {
            title: '0.5 HP Espa Prisma 15 2M Pressure Pump (Made in China)',
            slug: '0-5hp-espa-prisma-15-2m-pressure-pump-made-in-china',
            categoryName: 'Pressure Pumps',
            description:
               'Prisma 15 2M (Made in China)\nSame specifications as original model but sourced from China.',
            imageUrl: '/centrifugal/espa_0.8hp.png',
            price: 40000,
            discountPrice: null,
            stock: 10,
            brand: 'Espa',
            status: 'active',
            isFeatured: false,
            specs: [
               { id: 0, field: 'Horsepower', value: '0.5HP' },
               { id: 1, field: 'Motor Power (P2)', value: '0.24 kW' },
               { id: 2, field: 'Voltage', value: '230 V' },
               { id: 3, field: 'Protection', value: 'IPX5' },
               { id: 4, field: 'Max Suction', value: '2 m' },
               { id: 5, field: 'Max Working Pressure', value: '6 bar' },
               { id: 6, field: 'Discharge Size', value: '1\u2033' },
               { id: 7, field: 'Suction Size', value: '1\u2033' },
               { id: 8, field: 'Origin', value: 'Made in China' },
            ],
         },
         {
            title: '0.8 HP Espa Prisma 15 3M Pressure Pump',
            slug: '0-8hp-espa-prisma-15-3m-pressure-pump',
            categoryName: 'Pressure Pumps',
            description:
               'Prisma 15 series\n3M version: Higher head (30-5 bar range) with same materials and design features.\nSelf-priming to 2 m, IPX5, 1" suction/discharge.\nMotor power P2: 0.37 kW.',
            imageUrl: '/centrifugal/espa_0.8hp.png',
            price: 56000,
            discountPrice: null,
            stock: 10,
            brand: 'Espa',
            status: 'active',
            isFeatured: false,
            specs: [
               { id: 0, field: 'Horsepower', value: '0.8HP' },
               { id: 1, field: 'Motor Power (P2)', value: '0.37 kW' },
               { id: 2, field: 'Voltage', value: '230 V' },
               { id: 3, field: 'Protection', value: 'IPX5' },
               { id: 4, field: 'Max Suction', value: '2 m' },
               { id: 5, field: 'Max Working Pressure', value: '6 bar' },
               { id: 6, field: 'Discharge Size', value: '1\u2033' },
               { id: 7, field: 'Suction Size', value: '1\u2033' },
            ],
         },
         {
            title: '1.0 HP Espa Prisma 15 4M Pressure Pump (Made in China)',
            slug: '1-0hp-espa-prisma-15-4m-pressure-pump-made-in-china',
            categoryName: 'Pressure Pumps',
            description:
               'Prisma 15 4M (Made in China)\nSame performance and build specs as original 4M model.',
            imageUrl: '/centrifugal/espa_0.8hp.png',
            price: 40000,
            discountPrice: null,
            stock: 10,
            brand: 'Espa',
            status: 'active',
            isFeatured: false,
            specs: [
               { id: 0, field: 'Horsepower', value: '1.0HP' },
               { id: 1, field: 'Motor Power (P2)', value: '0.55 kW' },
               { id: 2, field: 'Voltage', value: '230 V' },
               { id: 3, field: 'Protection', value: 'IPX5' },
               { id: 4, field: 'Max Suction', value: '2 m' },
               { id: 5, field: 'Max Working Pressure', value: '6 bar' },
               { id: 6, field: 'Discharge Size', value: '1\u2033' },
               { id: 7, field: 'Suction Size', value: '1\u2033' },
               { id: 8, field: 'Origin', value: 'Made in China' },
            ],
         },
         {
            title: '0.8 HP Espa Prisma 15 3M Pressure Pump (Made in China)',
            slug: '0-8hp-espa-prisma-15-3m-pressure-pump-made-in-china',
            categoryName: 'Pressure Pumps',
            description:
               'Prisma 15 3M (Made in China)\nSame hydraulic & electrical specs as original 3M model.',
            imageUrl: '/centrifugal/espa_0.8hp.png',
            price: 40000,
            discountPrice: null,
            stock: 10,
            brand: 'Espa',
            status: 'active',
            isFeatured: false,
            specs: [
               { id: 0, field: 'Horsepower', value: '0.8HP' },
               { id: 1, field: 'Motor Power (P2)', value: '0.37 kW' },
               { id: 2, field: 'Voltage', value: '230 V' },
               { id: 3, field: 'Protection', value: 'IPX5' },
               { id: 4, field: 'Max Suction', value: '2 m' },
               { id: 5, field: 'Max Working Pressure', value: '6 bar' },
               { id: 6, field: 'Discharge Size', value: '1\u2033' },
               { id: 7, field: 'Suction Size', value: '1\u2033' },
               { id: 8, field: 'Origin', value: 'Made in China' },
            ],
         },
      ],
   },
   {
      title: 'Taifu TCP130 Centrifugal Pump 0.5HP \u2013 100% Copper Winding',
      slug: 'taifu-tcp130-centrifugal-pump-0.5hp-100percent-copper-winding',
      categoryName: 'Centrifugal Pumps',
      description:
         '0.5HP Centrifugal Pump\r\nDurable and efficient for household and agricultural applications.\r\n100% copper winding ensures reliability and long life.',
      imageUrl: '/centrifugal/Taifu TCP130.webp',
      price: 17000,
      discountPrice: null,
      stock: 10,
      brand: 'Taifu',
      status: 'active',
      isFeatured: false,
      specs: [
         { id: 0, field: 'Horsepower', value: '0.5HP' },
         { id: 1, field: 'Winding', value: '100% Copper' },
      ],
   },
   {
      title: 'Taifu TCP158 Centrifugal Pump 1.0HP \u2013 100% Copper Winding',
      slug: 'taifu-tcp158-centrifugal-pump-1.0hp-100percent-copper-winding',
      categoryName: 'Centrifugal Pumps',
      description:
         '1.0HP Centrifugal Pump\r\nRobust construction suitable for continuous operation.\r\n100% copper winding for enhanced motor life.',
      imageUrl: '/centrifugal/Taifu Tcp 158.webp',
      price: 22999,
      discountPrice: null,
      stock: 10,
      brand: 'Taifu',
      status: 'active',
      isFeatured: false,
      specs: [
         { id: 0, field: 'Horsepower', value: '1.0HP' },
         { id: 1, field: 'Winding', value: '100% Copper' },
      ],
   },
   {
      title: 'Taifu QB70 Peripheral Pump 0.75HP \u2013 100% Copper Winding',
      slug: 'taifu-qb70-peripheral-pump-0-75hp',
      categoryName: 'Pressure Pumps',
      description:
         'Peripheral Pump 0.75HP\nReliable for domestic use.\n100% copper winding for longer motor life.\nCompact, durable, and efficient design.',
      imageUrl: '/centrifugal/Taifu QB70.webp',
      price: 16999,
      discountPrice: null,
      stock: 10,
      brand: 'Taifu',
      status: 'active',
      isFeatured: false,
      specs: [
         { id: 0, field: 'Horsepower', value: '0.75HP' },
         { id: 1, field: 'Winding', value: '100% Copper' },
      ],
   },
   {
      title: 'Taifu TCP200 Centrifugal Pump 2.0HP \u2013 100% Copper Winding',
      slug: 'taifu-tcp200-centrifugal-pump-2.0hp-100percent-copper-winding',
      categoryName: 'Centrifugal Pumps',
      description:
         '2.0HP Centrifugal Pump\r\nDurable design for continuous duty.\r\n100% copper winding ensures long life and efficiency.\r\nIdeal for water supply, irrigation, and pressure boosting.',
      imageUrl: '/centrifugal/taifu_tcp200.webp',
      price: 32999,
      discountPrice: null,
      stock: 10,
      brand: 'Taifu',
      status: 'active',
      isFeatured: false,
      specs: [
         { id: 0, field: 'Horsepower', value: '2.0HP' },
         { id: 1, field: 'Winding', value: '100% Copper' },
      ],
   },
   {
      title: 'Taifu QB60 Peripheral Pump 0.5HP \u2013 100% Copper Winding',
      slug: 'taifu-qb60-peripheral-pump-0.5hp-100percent-copper-winding',
      categoryName: 'Pressure Pumps',
      description:
         'Peripheral Pump 0.5HP\r\nReliable domestic pump for small-scale water transfer.\r\n100% copper winding for durability and efficiency.',
      imageUrl: '/centrifugal/taifu QB60.jpg',
      price: 11499,
      discountPrice: null,
      stock: 10,
      brand: 'Taifu',
      status: 'active',
      isFeatured: false,
      specs: [
         { id: 0, field: 'Horsepower', value: '0.5HP' },
         { id: 1, field: 'Winding', value: '100% Copper' },
      ],
   },
   {
      title: 'Taifu QB80 Peripheral Pump 1.0HP \u2013 100% Copper Winding',
      slug: 'taifu-qb80-peripheral-pump-1.0hp-100percent-copper-winding',
      categoryName: 'Pressure Pumps',
      description:
         'Peripheral Pump 1.0HP\r\nConnection 1"\u00d71"  Max Suction: 8M  45 L/min\r\n1HP QB80 \u2013 Peripheral Pump with 100% copper winding.\r\nReliable for domestic water transfer with good suction and flow rates.',
      imageUrl: '/centrifugal/Taifu QB80.webp',
      price: 18499,
      discountPrice: null,
      stock: 10,
      brand: 'Taifu',
      status: 'active',
      isFeatured: false,
      specs: [
         { id: 0, field: 'Horsepower', value: '1.0HP' },
         { id: 1, field: 'Connection', value: '1"\u00d71"' },
         { id: 2, field: 'Max Suction', value: '8M' },
         { id: 3, field: 'Flow Rate', value: '45 L/min' },
         { id: 4, field: 'Winding', value: '100% Copper' },
      ],
   },
   {
      title: 'Taifu QB90 Peripheral Pump 1.2HP \u2013 100% Copper Winding',
      slug: 'taifu-qb90-peripheral-pump-1.2hp-100percent-copper-winding',
      categoryName: 'Pressure Pumps',
      description:
         '1.2HP Peripheral Pump\r\nConnection 1"x 1"  Max Suction: 8M  Flow Rate: 42 L/M  Max Head: 75M\r\nProduced by advanced high-speed punching line.\r\nAdopting DE carbon mechanical seal, prolong service life 170%.\r\nClass-F copper wire, motor heat resistance up to 155\u00b0C.\r\nAnti-rust, wear-resistant and high precision.\r\n100% rotor fault detection test.\r\nWelded stainless steel rotor shaft.\r\nThickened and reinforced, stable and durable.\r\nCooling sink area increased by 20%.\r\nThickness of motor body and foot increased by 20%.\r\nAdopting Japanese-imported casting line.\r\nHigh strength and corrosion resistant.\r\n100% electrophoresis treatment for anti-rust.\r\nAnti-strike and anti-aging.\r\nReinforcing nylon material.\r\nAnti-drop test 0.8 meters withstand 500g.\r\nBrass.',
      imageUrl: '/centrifugal/Taifu QB90.webp',
      price: 21999,
      discountPrice: null,
      stock: 10,
      brand: 'Taifu',
      status: 'active',
      isFeatured: false,
      specs: [
         { id: 0, field: 'Horsepower', value: '1.2HP' },
         { id: 1, field: 'Connection', value: '1"x 1"' },
         { id: 2, field: 'Max Suction', value: '8M' },
         { id: 3, field: 'Flow Rate', value: '42 L/M' },
         { id: 4, field: 'Max Head', value: '75M' },
         { id: 5, field: 'Winding', value: '100% Copper' },
      ],
   },
   {
      title: 'Pioneer HKS60 Self Priming Pump',
      slug: 'pioneer-hks60-self-priming-pump',
      categoryName: 'Self-Priming Pumps',
      description:
         '0.5HP\r\nApplication: Suitable for use with clean water that does not contain abrasive particles and liquids that are not chemically aggressive towards the materials.\r\nCompact, reliable, and easy to use for domestic applications such as water distribution, garden irrigation, and drawing water from tanks even when air or water may be present.\r\nComes complete with a flap-check valve.\r\nOperating conditions: liquid temperature up to 60\u2103, ambient temperature up to 40\u2103, total suction lift up to 9m.',
      imageUrl: '/centrifugal/pioneer_hks60.webp',
      price: 13500,
      discountPrice: null,
      stock: 10,
      brand: 'Pioneer',
      status: 'active',
      isFeatured: false,
      specs: [
         { id: 0, field: 'Horsepower', value: '0.5HP' },
         { id: 1, field: 'Total Head', value: '30 meter' },
         { id: 2, field: 'Total Suction Lift', value: '9m' },
      ],
   },
   {
      title: 'Pioneer HKS70 Self Priming Pump',
      slug: 'pioneer-hks70-self-priming-pump',
      categoryName: 'Self-Priming Pumps',
      description:
         '0.7HP\r\nApplication: Suitable for use with clean water that does not contain abrasive particles and liquids that are not chemically aggressive towards the materials.\r\nCompact, reliable, and easy to use in domestic applications such as water distribution, garden irrigation, and drawing water from tanks even when air may be present.\r\nComes complete with a flap-check valve.\r\nOperating conditions:\r\nLiquid temperature up to 60\u2103\r\nAmbient temperature up to 40\u2103\r\nTotal suction lift up to 9m.',
      imageUrl: '/centrifugal/Pioneer HKS70.webp',
      price: 18500,
      discountPrice: null,
      stock: 10,
      brand: 'Pioneer',
      status: 'active',
      isFeatured: false,
      specs: [
         { id: 0, field: 'Horsepower', value: '0.7HP' },
         { id: 1, field: 'Total Suction Lift', value: '9m' },
      ],
   },
   {
      title: 'Pedrollo PKm 60 (Made in Italy)',
      slug: 'pedrollo-pkm-60-made-in-italy',
      categoryName: 'Centrifugal Pumps',
      description:
         'Peripheral impeller pump\nDesigned to transfer clean water free from abrasive particles and liquids that will not damage the pump\u2019s components.\nReliable, easy to operate, and cost-effective.\nIdeal for domestic applications, specifically for gardens and orchards, in combination with small autoclave tanks.',
      imageUrl: '/centrifugal/Pedrollo PKm 60.webp',
      price: 28500,
      discountPrice: null,
      stock: 10,
      brand: 'Pedrollo',
      status: 'active',
      isFeatured: false,
      specs: [
         { id: 0, field: 'Flow Rate', value: 'up to 40 l/min (2.4 m\u00b3/h)' },
         { id: 1, field: 'Maximum Head', value: 'up to 40m' },
         { id: 2, field: 'Manometric Suction Head', value: 'up to 8m' },
         { id: 3, field: 'Liquid Temperature', value: '-10\u00b0C to +60\u00b0C' },
         { id: 4, field: 'Ambient Temperature', value: 'up to +50\u00b0C' },
         { id: 5, field: 'Maximum Working Pressure', value: '6 bar' },
         { id: 6, field: 'Motor', value: '2 poles' },
         { id: 7, field: 'Ingress Protection Rating', value: 'IP X4' },
         { id: 8, field: 'Pump Housing', value: 'Cast iron' },
         { id: 9, field: 'Impeller', value: 'Brass' },
         { id: 10, field: 'Shaft', value: 'Stainless steel AISI 431' },
         { id: 11, field: 'Motor Bracket', value: 'Aluminium with brass cover' },
      ],
   },
   {
      title: 'Pedrollo CP158 ST6 Centrifugal Electric Pump (Stainless Steel, 3-Phase)',
      slug: 'pedrollo-cp158-st6-centrifugal-electric-pump',
      categoryName: 'Centrifugal Pumps',
      description:
         'Stainless steel centrifugal pump, 3-phase CP-ST6 series\nSuitable for clean water and non-aggressive liquids.\nAll wet parts in stainless steel AISI 316L for high corrosion resistance.\nUse: domestic, agricultural, industrial water transfer.\nManometric suction lift up to 7 m; liquid temperature -10 \u00b0C to +90 \u00b0C; ambient max +40 \u00b0C.\nProtection: IP X4. Continuous duty (S1).',
      imageUrl: '/centrifugal/Pedrolla 158.webp',
      price: 19000,
      discountPrice: null,
      stock: 10,
      brand: 'Pedrollo',
      status: 'active',
      isFeatured: false,
      specs: [
         { id: 0, field: 'Phase', value: '3-phase' },
         { id: 1, field: 'Horsepower', value: '1 HP' },
         { id: 2, field: 'FlowRate', value: '10 \u00f7 120 l/min' },
         { id: 3, field: 'HeadRange', value: '35.5 \u00f7 19 m' },
         { id: 4, field: 'Max Working Pressure', value: '6 bar' },
         { id: 5, field: 'Manometric Suction', value: '7 m' },
         { id: 6, field: 'Liquid Temp Range', value: '-10 \u00b0C to +90 \u00b0C' },
         { id: 7, field: 'Ambient Temp Max', value: '+40 \u00b0C' },
         { id: 8, field: 'Pump Housing Material', value: 'Stainless Steel AISI 316L' },
         { id: 9, field: 'Impeller Material', value: 'Stainless Steel AISI 316L' },
         { id: 10, field: 'Shaft Material', value: 'Stainless Steel 316L' },
      ],
   },
]

export const accessoriesSeed = [
   {
      title: 'ESPA Type Pressure Control PC-81 220V',
      slug: 'espa-type-pressure-control-pc-81-220v',
      description:
         'ESPA Type Pressure Control PC-81 automatic pressure control kit. Starts at 1.5 or 2.2 bar, suitable for 110V/220V systems.',
      imageUrl: '/centrifugal/espa-pressure-kit.jpg',
      price: 8498,
      discountPrice: null,
      stock: 0,
      brand: 'ESPA',
      status: 'active',
      specs: [
         { id: 0, field: 'startingPressure', value: ['1.5 bar', '2.2 bar'] },
         { id: 1, field: 'maxAllowablePressure', value: '10 bar' },
         { id: 2, field: 'maxRatedCurrent', value: '10 A' },
         { id: 3, field: 'voltage', value: ['110 V', '220 V'] },
         { id: 4, field: 'frequency', value: '50/60 Hz' },
         { id: 5, field: 'protectionDegree', value: 'IP54' },
         { id: 6, field: 'ambientTemperatureMax', value: '40°C' },
         { id: 7, field: 'liquidTemperatureMax', value: '60°C' },
      ],
   },
]

export const categoriesSeedData = [
   {
      name: 'Centrifugal Pumps',
      slug: 'centrifugal-pumps',
      isFeatured: true,
      description:
         'Durable centrifugal pumps for residential, commercial, and industrial water supply.',
   },
   {
      name: 'Circulating Pumps',
      slug: 'circulating-pumps',
      isFeatured: false,
      status: 'active',
      description: 'Efficient circulating pumps for heating, cooling, and water systems.',
   },
   {
      name: 'Pressure Pumps',
      slug: 'pressure-pumps',
      isFeatured: false,
      status: 'active',
      description:
         'Reliable pressure pumps to maintain steady water flow in households and industries.',
   },
   {
      name: 'Self-Priming Pumps',
      slug: 'self-priming-pumps',
      isFeatured: false,
      status: 'active',
      description: 'Self-priming pumps designed for easy operation and reliable performance.',
   },
   {
      name: 'Submersible Pumps and Motors',
      slug: 'submersible-pumps-and-motors',
      isFeatured: true,
      description: 'High-performance submersible pumps and motors for deep water extraction.',
   },
   {
      name: 'Submersible Sewage Pumps',
      slug: 'submersible-sewage-pumps',
      isFeatured: false,
      status: 'active',
      description: 'Durable sewage pumps for waste management and heavy-duty drainage.',
   },
   {
      name: 'High Pressure Washers',
      slug: 'high-pressure-washers',
      isFeatured: false,
      status: 'active',
      description: 'Powerful high-pressure washers for cleaning applications.',
   },
   {
      name: 'Swimming Pool Pumps',
      slug: 'swimming-pool-pumps',
      isFeatured: false,
      status: 'active',
      description: 'Reliable swimming pool pumps for water circulation and cleaning.',
   },
   {
      name: 'Chemical Dosing Pumps',
      slug: 'chemical-dosing-pumps',
      isFeatured: false,
      status: 'active',
      description: 'Accurate chemical dosing pumps for water treatment and industrial use.',
   },
   {
      name: 'Fountain Pumps',
      slug: 'fountain-pumps',
      isFeatured: false,
      status: 'active',
      description: 'Decorative fountain pumps for gardens, landscapes, and water features.',
   },
]
